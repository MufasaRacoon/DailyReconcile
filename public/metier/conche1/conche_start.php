...
// exit;
    ...
    $result = $controller_conche->createStart($_POST, $_SESSION['user_id'], 'Conche 1', true);
...
<head>
    ...
    <title>Conche 1 - Smart Factory</title>
...
<body>
    ...
    <div class="glass-container">
        <header style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 15px;">
            <h1>NOUVEAU CHARGEMENT DANS CONCHE 1</h1>
        ...
        <main>
    <!-- Formulaire CONCHE 1 OF -->
            <form action="conche_start.php" method="post" class="section-card">
                <div class="grid-2">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" name="of_code_principale" placeholder="Code de Fabrication" required>
    ...
    <!-- FORMULAIRE BATCH -->
                <div class="input-group" style="margin-bottom: 25px;">
                    ...
                    <div class="input-wrapper">
                        <input type="number" name="qty_principale" placeholder="Ex: 500" required>
                        <span>kg</span>
                    </div>
                </div>

                <div class="grid-2">
                    <div class="input-group"><label>Cocoa Nibs</label><div class="input-wrapper"><input type="number" name="qty_cocoa_nibs" id="cocoa-nibs"><span>kg</span></div></div>
                    <div class="input-group"><label>Sugar Cane</label><div class="input-wrapper"><input type="number" name="qty_sugar_cane" id="sugar-cane"><span>kg</span></div></div>
                    <div class="input-group"><label>Cocoa Butter</label><div class="input-wrapper"><input type="number" name="qty_cocoa_butter" id="cocoa-butter"><span>kg</span></div></div>
                    <div class="input-group"><label>Lecithin</label><div class="input-wrapper"><input type="number" name="qty_lecithin" id="lecithin"><span>kg</span></div></div>
                </div>

                <div class="stock-display">
                    <label style="color: rgba(255,255,255,0.6);">Stock Actuel en Conche</label>
                    <div class="total-value">
                    <input type="number" name="qty_stock" id="stock-conche" style="text-align: center; font-size: 2rem; font-weight: 800; color: var(--success); background-color: var(--text-main)" required>    
                    kg
                    </div>
                </div>
    ...
    <script>
        const m1 = document.getElementById('cocoa-nibs');
        const m2 = document.getElementById('sugar-cane');
        const m3 = document.getElementById('cocoa-butter');
        const m4 = document.getElementById('lecithin');
        const stock = document.getElementById('stock-conche');
    ...
    // Calcule automatique du stock en conche (Somme des ingredients)
        function calculSomme(){
            // On convertit les valeurs en nombres flottants (0 par défaut si vide)
            const v1 = parseFloat(m1.value) || 0;
            const v2 = parseFloat(m2.value) || 0;
            const v3 = parseFloat(m3.value) || 0;
            const v4 = parseFloat(m4.value) || 0;

            const inputs = [m1.value, m2.value, m3.value, m4.value].filter(val => val !== "" && !isNaN(val));
            if (inputs.length > 0) {
                const somme = v1 + v2 + v3 + v4;
                
                // On affiche le résultat avec 2 décimales pour la précision micrométrique
                stock.value = somme.toFixed(2);
            } else {
                stock.value = "";
            }
        }
    ...
    // On écoute l'événement 'input' sur les 3 champs
        [m1, m2, m3, m4].forEach(input => {
            input.addEventListener('input', calculSomme);
        });
