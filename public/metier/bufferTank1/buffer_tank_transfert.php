...
$is_exist = $controller_buffer_tank->find($id);
if (!$is_exist) {
    die('données introuvable');
}

// $equipment_id = $controller_conche->getIdConche1();
$data = $controller_buffer_tank->get_batch($controller_buffer_tank->getIdBufferTank1());
$ofList = $controller_buffer_tank->getSelectableOF($controller_buffer_tank->getIdBufferTank1());
$isExist_sous_of = $controller_buffer_tank->charge_sous_of($id);

// Data Equipment
$dataChocoTank1 = $controller_buffer_tank->getEquipment($controller_buffer_tank->getIdChocoTank1());
$dataChocoTank2 = $controller_buffer_tank->getEquipment($controller_buffer_tank->getIdChocoTank2());
$dataBufferTank2 = $controller_buffer_tank->getEquipment($controller_buffer_tank->getIdBufferTank2());
$dataSelmiTank1 = $controller_buffer_tank->getEquipment($controller_buffer_tank->getIdSelmiTank1());
$dataSelmiTank2 = $controller_buffer_tank->getEquipment($controller_buffer_tank->getIdSelmiTank2());
$dataSelmiTank3 = $controller_buffer_tank->getEquipment($controller_buffer_tank->getIdSelmiTank3());
$dataConche2 = $controller_buffer_tank->getEquipment($controller_buffer_tank->getIdConche2());
$dataConche1 = $controller_buffer_tank->getEquipment($controller_buffer_tank->getIdConche1());

if($_SERVER['REQUEST_METHOD'] === 'POST'){
    $result = $controller_buffer_tank->transfert($id, $_POST, $_SESSION['user_id']);
    if ($result === true){
        header('Location: ../../buffer_tank_1.php');
        exit;
    } else {
        $error = $result;
    }
...
<head>
    ...
    <title>Transfert Depuis Buffer Tank 1</title>
    <style>
        :root {
...
<body>
    ...
    <div class="glass-container">
        <header style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 15px;">
            <h1>TRANSFERT DEPUIS Buffer Tank 1</h1>
            <button class="btn accent" onclick="location.href='../../buffer_tank_1.php'" style="width: auto;">Annuler</button>
        ...
        <main>
            ...
            <form id="transferForm" action="buffer_tank_transfert.php?id=<?= (int)$data['id'] ?>" method="post" class="section-card" style="border-left: 5px solid var(--accent);">
                
                <input type="hidden" name="equipment_id" value='<?= htmlspecialchars((string)$data['equipment_id']) ?>'>

                <div class="stock-display">
                    <label style="color: rgba(255,255,255,0.6);">Stock Actuel en Buffer Tank 1</label>
                ...
                <div class="grid-3">
                    ...
                    <div class="input-group">
                        ...
                        <select name="destination" id="selectDestination" required>
                            <option value="">Sélectionner...</option>
                            <option value="<?= htmlspecialchars($controller_buffer_tank->getIdChocoTank1()) ?>">Choco Tank 1 - <?= htmlspecialchars((string)$dataChocoTank1['current_qty_kg']) ?> Kg / <?= htmlspecialchars((string)$dataChocoTank1['capacity_kg']) ?> Kg</option>
                            <option value="<?= htmlspecialchars($controller_buffer_tank->getIdChocoTank2()) ?>">Choco Tank 2  - <?= htmlspecialchars((string)$dataChocoTank2['current_qty_kg']) ?> Kg / <?= htmlspecialchars((string)$dataChocoTank2['capacity_kg']) ?> Kg</option>
                            <option value="<?= htmlspecialchars($controller_buffer_tank->getIdBufferTank2()) ?>">Buffer Tank 2 - <?= htmlspecialchars((string)$dataBufferTank2['current_qty_kg']) ?> Kg / <?= htmlspecialchars((string)$dataBufferTank2['capacity_kg']) ?> Kg</option>
                            <option value="<?= htmlspecialchars($controller_buffer_tank->getIdSelmiTank1()) ?>">Selmi Tank 1 - <?= htmlspecialchars((string)$dataSelmiTank1['current_qty_kg']) ?> Kg / <?= htmlspecialchars((string)$dataSelmiTank1['capacity_kg']) ?> Kg</option>
                            <option value="<?= htmlspecialchars($controller_buffer_tank->getIdSelmiTank2()) ?>">Selmi Tank 2 - <?= htmlspecialchars((string)$dataSelmiTank2['current_qty_kg']) ?> Kg / <?= htmlspecialchars((string)$dataSelmiTank2['capacity_kg']) ?> Kg</option>
                            <option value="<?= htmlspecialchars($controller_buffer_tank->getIdSelmiTank3()) ?>">Selmi Tank 3 - <?= htmlspecialchars((string)$dataSelmiTank3['current_qty_kg']) ?> Kg / <?= htmlspecialchars((string)$dataSelmiTank3['capacity_kg']) ?> Kg</option>
                            <option value="<?= htmlspecialchars($controller_buffer_tank->getIdConche2()) ?>">Conche 2 - <?= htmlspecialchars((string)$dataConche2['current_qty_kg']) ?> Kg / <?= htmlspecialchars((string)$dataConche2['capacity_kg']) ?> Kg</option>
                            <option value="<?= htmlspecialchars($controller_buffer_tank->getIdConche1()) ?>">Conche 1 - <?= htmlspecialchars((string)$dataConche1['current_qty_kg']) ?> Kg / <?= htmlspecialchars((string)$dataConche1['capacity_kg']) ?> Kg</option>
                        </select>
                    </div>
                </div>
