...
$error = null;
$id = (int)($_GET['id'] ?? 0);
$controller_selmi_tank = new MachineController($pdo);

$is_exist = $controller_selmi_tank->find($id);
if (!$is_exist) {
    die('données introuvable');
}

$data = $controller_selmi_tank->get_batch($controller_selmi_tank->getIdSelmiTank1());
$ofList = $controller_selmi_tank->getSelectableOF($controller_selmi_tank->getIdSelmiTank1());
$isExist_sous_of = $controller_selmi_tank->charge_sous_of($id);

// Data Equipment
$dataChocoTank1 = $controller_selmi_tank->getEquipment($controller_selmi_tank->getIdChocoTank1());
$dataChocoTank2 = $controller_selmi_tank->getEquipment($controller_selmi_tank->getIdChocoTank2());
$dataBufferTank1 = $controller_selmi_tank->getEquipment($controller_selmi_tank->getIdBufferTank1());
$dataBufferTank2 = $controller_selmi_tank->getEquipment($controller_selmi_tank->getIdBufferTank2());
$dataSelmiTank2 = $controller_selmi_tank->getEquipment($controller_selmi_tank->getIdSelmiTank2());
$dataSelmiTank3 = $controller_selmi_tank->getEquipment($controller_selmi_tank->getIdSelmiTank3());
$dataConche2 = $controller_selmi_tank->getEquipment($controller_selmi_tank->getIdConche2());
$dataConche1 = $controller_selmi_tank->getEquipment($controller_selmi_tank->getIdConche1());

if($_SERVER['REQUEST_METHOD'] === 'POST'){
    $result = $controller_selmi_tank->transfert($id, $_POST, $_SESSION['user_id']);
    if ($result === true){
        header('Location: ../../selmi_tank_1.php');
        exit;
    } else {
        $error = $result;
    }
...
<head>
    ...
    <title>Transfert Depuis Selmi Tank 1</title>
    <style>
        :root {
...
<body>
    ...
    <div class="glass-container">
        <header style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 15px;">
            <h1>TRANSFERT DEPUIS Selmi Tank 1</h1>
            <button class="btn accent" onclick="location.href='../../selmi_tank_1.php'" style="width: auto;">Annuler</button>
        ...
        <main>
            ...
            <form id="transferForm" action="selmi_tank_transfert.php?id=<?= (int)$data['id'] ?>" method="post" class="section-card" style="border-left: 5px solid var(--accent);">
                
                <input type="hidden" name="equipment_id" value='<?= htmlspecialchars((string)$data['equipment_id']) ?>'>

                <div class="stock-display">
                    <label style="color: rgba(255,255,255,0.6);">Stock Actuel en Selmi Tank 1</label>
                ...
                <div class="grid-3">
                    ...
                    <div class="input-group">
                        ...
                        <select name="destination" id="selectDestination" required>
                            <option value="">Sélectionner...</option>
                            <option value="<?= htmlspecialchars($controller_selmi_tank->getIdChocoTank1()) ?>">Choco Tank 1 - <?= htmlspecialchars((string)$dataChocoTank1['current_qty_kg']) ?> Kg / <?= htmlspecialchars((string)$dataChocoTank1['capacity_kg']) ?> Kg</option>
                            <option value="<?= htmlspecialchars($controller_selmi_tank->getIdChocoTank2()) ?>">Choco Tank 2  - <?= htmlspecialchars((string)$dataChocoTank2['current_qty_kg']) ?> Kg / <?= htmlspecialchars((string)$dataChocoTank2['capacity_kg']) ?> Kg</option>
                            ...
                            <option value="<?= htmlspecialchars($controller_selmi_tank->getIdSelmiTank3()) ?>">Selmi Tank 3 - <?= htmlspecialchars((string)$dataSelmiTank3['current_qty_kg']) ?> Kg / <?= htmlspecialchars((string)$dataSelmiTank3['capacity_kg']) ?> Kg</option>
