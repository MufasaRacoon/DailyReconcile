...
$data = $controller_choco_tank->get_batch($controller_choco_tank->getIdChocoTank2());
$ofList = $controller_choco_tank->getSelectableOF($controller_choco_tank->getIdChocoTank2());
$isExist_sous_of = $controller_choco_tank->charge_sous_of($id);

// Data Equipment
$dataChocoTank1 = $controller_choco_tank->getEquipment($controller_choco_tank->getIdChocoTank1());
...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if ($result === true){
        header('Location: ../../chocolat_tank_2.php');
        exit;
    } else {
        $error = $result;
    }
...
<head>
    ...
    <title>Transfert Depuis Chocolat Tank 2</title>
    <style>
        :root {
...
<body>
    ...
    <div class="glass-container">
        <header style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 15px;">
            <h1>TRANSFERT DEPUIS Chocolat Tank 2</h1>
            <button class="btn accent" onclick="location.href='../../chocolat_tank_2.php'" style="width: auto;">Annuler</button>
                ...
                <div class="grid-3">
                    ...
                    <div class="input-group">
                        ...
                        <select name="destination" id="selectDestination" required>
                            <option value="">Sélectionner...</option>
                            <option value="<?= htmlspecialchars($controller_choco_tank->getIdChocoTank1()) ?>">Choco Tank 1 - <?= htmlspecialchars((string)$dataChocoTank1['current_qty_kg']) ?> Kg / <?= htmlspecialchars((string)$dataChocoTank1['capacity_kg']) ?> Kg</option>
