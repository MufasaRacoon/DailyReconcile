...
requireAuth();
require_once __DIR__ . '/../../../app/controllers/Chocolat_tank1_Controller.php';

$error = null;
$id = (int)($_GET['id'] ?? 0);
$controller_choco_tank = new Chocolat_tank1_Controller($pdo);
...
$ofList = $controller_choco_tank->getSelectableOF();
...
<body>
    ...
    <nav class="navbar">
        <div class="nav-logo">AMCHO // <span>Smart Factory</span></div>
    ...
    <div class="glass-container">
        ...
        <main>
            <?php echo $error; ?>
            <form action="chocolat_tank_transfert.php?id=<?= (int)$data['id'] ?>'" method="post" class="section-card" style="border-left: 5px solid var(--accent);">
                <div class="stock-display">
                <label style="color: rgba(255,255,255,0.6);">Stock Actuel en Chocolat Tank</label>
                ...
                <input type="hidden" name="current_qty_kg" value='<?= htmlspecialchars((string)$data['current_qty_kg']) ?>'>
                <label style="color: rgba(255,255,255,0.6);">OF :</label>
                <div class="total-value"><?= htmlspecialchars((string)$data['of_code_principale']) ?></div>
                <input type="hidden" name="of_code_principale" value='<?= htmlspecialchars((string)$data['of_code_principale']) ?>'>
                ...
                <input type="hidden" name="type_chocolat" value='<?= htmlspecialchars((string)$data['type_chocolat']) ?>'>
                </div>
                <div class="grid-3">
                    <?php if(!empty($isExist_sous_of)){ ?>
                    <div class="input-group">
                        <label>Sous-OF Cible  <span style="color:red; font-size: 16px">*</span></label>
                        <select name="of_code" required>
                            <option value="">-- Sélectionner --</option>

                            <!-- Liste des Sous OF entrer -->
                    ...
                    <?php } ?>
                    <input type="hidden" name="of_code" value=''></div>
                    <div class="input-group">
                        <label>Poids à transférer  <span style="color:red; font-size: 16px">*</span></label> 
                        <div class="input-wrapper">
                            <input type="number" name="poids_transfert" required><span>kg</span>
                            <input type="hidden" name="equipment_id" value='<?= htmlspecialchars((string)$data['equipment_id']) ?>'>
                            <input type="hidden" name="qty_stock" value='<?= htmlspecialchars((string)$data['qty_stock']) ?>'>
                            <input type="hidden" name="of_code_principale" value='<?= htmlspecialchars((string)$data['of_code_principale']) ?>'>
                        </div>
                    </div>
                    <div class="input-group">
                ...
                <button type="submit" class="btn accent">Exécuter le Transfert</button>
            </form>
        </main>
    </div>
</body>
