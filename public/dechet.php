...
$controller_Dechet = new MachineController($pdo);
$data = $controller_Dechet->get_batch($controller_Dechet->getIdDechet());
...
<head>
    ...
    <title>Selmi Tank 1 - Smart Factory</title>
...
<?php if (!$hasStock) {  ?>
        <main>
            <div class="stock-display">
                ...
                <!-- <button class="btn accent" onclick="location.href='metier/conche1/conche_start.php'" style="margin-top:20px;">Nouveau Chargement</button> -->
        </main>

<!-- S'IL Y A DE BACTH EN COURS DANS DECHET -->
<?php } else { ?>
        <main>
            <div class="stock-display">
                <label style="color: rgba(255,255,255,0.6);">Quantité actuelle</label>
                <div class="total-value"><?= htmlspecialchars((string)$data['current_qty_kg']) ?> kg </div>
            <?php if ($data['status'] == 'STARTED_LOADING') { ?>
                <button class="btn accent" onclick="location.href='metier/selmiTank1/conche_loading.php?id=<?= (int)$data['id'] ?>'" style="margin-top:20px;">Chargement</button>
            <?php } else if($data['status'] == 'FINISHED_LOADING') { ?>
                <button class="btn accent" onclick="location.href='metier/selmiTank1/Selmi_tank_transfert.php?id=<?= (int)$data['id'] ?>'" style="margin-top:20px;">Lancer le transfert</button>
            <?php } else { ?>
                <button class="btn accent" onclick="location.href='metier/selmiTank1/Selmi_tank_transfert.php?id=<?= (int)$data['id'] ?>'" style="margin-top:20px;">Lancer le transfert</button>
               <?php }?>
        </main>
